...
class ConvolutionalEncoder:
    ...
    def network_guts(self):
        ...
        mu = conv_layers.fully_connected(self.representation_size, activation_fn=None, name = "enc_fc1")
        stddev_log_sq = conv_layers.fully_connected(self.representation_size, activation_fn=None, name = "enc_fc2")
...
class DeconvolutionalDecoder:
    ...
    def generate_network_random_sample(self, batch_size = None):
        batch_size = self._determine_batch_size(batch_size)
        epsilon = tf.random_normal([batch_size, self.representation_size])
        self.latent_var = epsilon
        return self.network_guts(batch_size)
